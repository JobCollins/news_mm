...
class NewsFetcher:
    ...
    def fetch_from_newsapi(self) -> List[Dict]:
        ...
        return [
            NewsAPIModel(**article).to_common() for article in response.get("articles", [])
        ]
    
    @handle_article_fetching
    def fetch_from_newsdataapi(self) -> List[Dict]:
        """
        fetch news data from the NewsDataAPI
        """
        ...
        return [
            NewsDataIOModel(**article).to_common() for article in response.get("results", [])
        ]
